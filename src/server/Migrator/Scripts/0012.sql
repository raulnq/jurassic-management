...
AS
SELECT cp.CollaboratorId, c.Name, cp.NetSalary, cp.Number, cp.Currency, cp.PaidAt as Date, null as Start, null as [End], -1 as Sign
	FROM $schema$.[CollaboratorPayments] cp
	JOIN $schema$.[Collaborators] c on c.CollaboratorId=cp.CollaboratorId
	WHERE Status in ('Paid', 'Confirmed')
UNION ALL
SELECT wi.CollaboratorId, c.Name, wi.SubTotal-wi.ProfitAmount, p.Number, p.Currency, CONVERT(DATE, p.IssuedAt) as Date, w.Start, w.[End], 1 as Sign
